...
class GuardianControlServer:
    ...
    DEFAULT_PORT = 14242  # fixed high port for easy agent discovery (no random ports)
    ...
    def start(self):
        ...
        # Bind the handler class to this instance for state access
        handler_class = type(
        ...
        try:
            self.httpd = ThreadedHTTPServer(("127.0.0.1", self.port), handler_class)
            ...
            self.logger.error(f"Failed to start local control interface on port {self.port}: {e}")
            self.httpd = None
...
class GuardianHandler(FileSystemEventHandler or object):
    ...
    def _evaluate(self, path: Path, event_type: str):
        try:
            if not path.exists():
                return
            if any(x in str(path) for x in ["__pycache__", ".git", "node_modules", ".deadpush"]):
                return
...
def run_guardian(intervention: bool = True, daemon: bool = False, strict: bool = False):
    ...
    # Start the Local Control Interface (AGENT priority 4) so AI agents can query/interact autonomously.
    # Started for both foreground and daemon (very useful for agents even in interactive `guard`).
    control_server = None
    try:
        control_port = getattr(config, "control_port", None) or GuardianControlServer.DEFAULT_PORT
        control_server = GuardianControlServer(handler, port=control_port)
        control_server.start()
        # Write discoverable port file so agents / other tools can find the interface without hardcoding.
        port_file = pid_dir / "guardian.control.port"
        port_file.write_text(str(control_server.port))
        logger.info(f"Local control interface on http://127.0.0.1:{control_server.port} (port file: {port_file})")
        atexit.register(control_server.stop)
    except Exception as e:
        logger.warning(f"Local control interface could not be started: {e} (agents fall back to `deadpush status` / CLI)")
...
def setup_autostart(repo_root: Path) -> str:
    ...
    import sys as _sys
    home = Path.home()
    exe = _sys.executable  # use the exact python that has deadgraph installed
...
[Service]
Type=simple
ExecStart={exe} -m deadgraph.cli guard --daemon
Restart=always
RestartSec=5
...
You can still achieve "survive reboot" by:
  ...
  - Using your distro's service manager manually pointing at: {exe} -m deadgraph.cli guard --daemon
