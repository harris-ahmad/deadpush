...
from .graph import CallGraph, DeadSymbol, DebrisFile, Edge, Symbol, make_symbol_id
...
def _run_full_analysis(config, explicit_entries=None, max_depth=-1, use_rich=True):
    ...
    for f in files:
        ...
        try:
            ...
            for sym in plugin.extract_symbols(tree, str(f.path)):
                graph.add_symbol(sym)

            # Build high-quality call graph using structured CallSite data
            file_symbols = {s.id: s for s in graph.symbols.values() if s.path == str(f.path)}
            file_imports = {}  # simple module -> names for resolution
            try:
                for imp in plugin.extract_imports(tree, str(f.path)):
                    if imp.module:
                        for n in imp.names:
                            if n != "*":
            ...
            for call in plugin.extract_call_sites(tree, str(f.path)):
                # Attempt to resolve callee to a known symbol id for high-quality edges
                resolved_id = _resolve_callee_to_symbol(
                    call, file_symbols, file_imports, graph.symbols, str(f.path)
                )
                target = resolved_id or call.callee or call.raw_callee_text
                # Use higher confidence for resolved calls
